...
export interface IndexerDependencies {
  indexChatMessage: (chatId: number, messageId: number, authorId: number, authorName?: string) => Promise<void>
}
...
export function buildIndexerHandler(deps: IndexerDependencies) {
  return async function indexMessage(message: IndexerMessage): Promise<void> {
    ...
    await deps.indexChatMessage(message.chat.id, message.message_id, from.id, from.username ?? from.first_name)
  }
}
