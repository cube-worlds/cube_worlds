...
export function saveImage(username: string, fileName: string, content: Buffer): string {
...
export function saveJSON(adminIndex: number, username: string, json: object): string {
  const jsonPath = path.join(folderPath(username), `${username}_${adminIndex}.json`)
