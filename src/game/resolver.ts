...
export interface ResolverDependencies {
  ...
  resolveVisitOnce: (visitId: string, payout: bigint, outcome: string, partnerPass?: number) => Promise<boolean>
  ...
  holdersOf: (userIds: number[]) => Promise<Map<number, HolderInfo>>
...
export function buildResolver(deps: ResolverDependencies) {
  async function runEngine(place: PlaceDef, visits: VisitRecord[], holders: Map<number, HolderInfo>): Promise<EngineResult> {
  ...
  async function settle(visit: VisitRecord, outcome: EngineOutcome, place: string, partner?: HolderInfo['pass']) {
    const text = partner ? `${outcome.outcome} · with ${partner.name}` : outcome.outcome
    const won = await deps.resolveVisitOnce(visit.id, outcome.payout, text, partner?.index)
    if (!won) return false
    ...
    try {
      await deps.notify(visit.userId, text, place)
    } catch (err) {
  ...
  async function resolveWindow(windowId: number): Promise<number> {
    ...
    const holders = await deps.holdersOf([...new Set(visits.map(v => v.userId))])
    ...
    for (const [placeId, placeVisits] of byPlace) {
      ...
      const result = await runEngine(place, placeVisits, holders)
      ...
      for (const outcome of result.outcomes) {
        ...
        const partner = outcome.partnerId === undefined ? undefined : holders.get(outcome.partnerId)?.pass
        if (await settle(visit, outcome, place.id, partner)) paid += 1
      }
