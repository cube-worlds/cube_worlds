...
const run = (visits: EngineVisit[]) => resolveStagHunt('Nusa Penida', visits, 100n, 1500n, 5000n, weightOf)
...
test('exactly the threshold brings the stag down: stake back plus a weight share of the pot', () => {
  ...
  assert.deepEqual(outcomes.map(o => [o.userId, o.payout]), [[1, 593n], [2, 593n], [3, 593n], [5, 120n]])
  assert.deepEqual(outcomes[0].rep, { helped: 1 })
  assert.equal(outcomes[0].outcome, 'Nusa Penida · 3 hunters · stag down · +593')
})
