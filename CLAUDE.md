...
5. **Gate (v4)** — entry to the game requires **holding a Cube Worlds NFT in the bound wallet**. `POST /api/pass/scan` lists the wallet's collection items via toncenter v3 (`pass.ts` composer, pure `pass-handler.ts`, cap 20); `POST /api/pass/select { index }` re-checks ownership (403 `not_owned`) and stores `User.pass` (`{ index, address, name, image, verifiedAt }`). Login returns `holder`/`pass`/`username` and revalidates ownership when `verifiedAt` is older than 1 h (gone → `$unset pass`; provider error → keep, log). Rebinding to a different wallet clears `pass`. `/api/mint/status` still drives the forge; minted users press ENTER WORLD I → scan. NFT images are **never loaded from a public IPFS gateway** (they 429 browsers): `passImageUrl()` in `login-payload.ts` rewrites any `ipfs://`/gateway URL to `GET /api/pass/image/*`, served by the disk cache in `ipfs-cache.ts` (pure, testable) wired in `pass.ts`. The cache tries gateways in order, sniffs magic bytes (that is also the "is it an image" check), dedupes concurrent fetches per CID, and is **bounded** — 8 MB/file, 256 MB total with oldest-first eviction — because the route is unauthenticated (an `<img>` src carries no initData) and is therefore a write primitive for any CID. `/pass/scan` and `/pass/select` warm the cache for the passes they return, so the picker does not wait on a cold gateway (~6 s first fetch, ~1 ms cached).

## Bali (the game, slices 1+2)
Every place is one game-theory engine reading the pass's 120 on-chain traits (`User.pass.traits`, fetched from the item content JSON at `/api/pass/select`, backfilled by `/api/world/state`). One `Visit` per holder per 8-hour window (`windowId = floor(now / 8h)`, UTC). Stake is debited on commit via `debitVotes` (`BalanceChangeType.Stake`), payout credited once at resolution (`Payout`; refunds are positive `Stake` rows). Places and numbers: `src/game/places.ts` (14 places, all open). Engines are pure, one file each in `src/game/engines/`: minority (Ubud/Batur/Lovina/Tanah Lot, no move), split-steal (Canggu: help|steal), commons (Besakih/Lembongan: give|take), hawk-dove (Kuta: hawk|dove), heist (Uluwatu: loyal|betray), all-pay (Seminyak: bid1|bid2|bid3 — the only player-chosen stake, fixed tiers via `stakeFor(place, move)`, `PlaceDef.bids`), volunteer (Amed: dive|wait), stag-hunt (Nusa Penida: stag|hare), ultimatum (Gili: fair|greedy|strict). Shared `engines/helpers.ts` (`pairVisits`, `shuffle`, `splitByWeight`). Every engine refunds (`refund: true`) a stake it cannot play; `partnerId` on an outcome makes the resolver append `· with <pass name>` and store `Visit.partnerPass`. `/api/world/state` exposes `moves` (and `bids`) per place so the frontend never hardcodes them. Resolver: `src/game/resolver.ts` (pure, DI) + `resolver-start.ts` (60 s tick, not in STAGING mode) — claims a `Window` lock, CAS-resolves each visit before paying (at-most-once), bumps `User.rep`, notifies via bot (non-fatal). API: `/api/world/state|visit|history` (holder-gated, 403 `holder_required`) and public `GET /api/world/pass/:index`. Deep links: `startapp=bali_<place>` and `startapp=meet_<inviteCode>` (Canggu pair invite). Admin `/resolve` forces the current window on staging/dev only.
...
## Critical Gotchas
- **Tests use Node.js built-in test runner** (`node --test`), not Jest/Vitest. 586 tests / 67 files; run `npm run test:coverage` for the report.
